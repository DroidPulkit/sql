...
SELECT * FROM customer ORDER_BY customer_last_name ASC, customer_first_name ASC
LIMIT 10;
...
SELECT vendor_booth_assignments.vendor_id, vendor.vendor_name, 
...
SELECT customer_id, strftime('%m', market_date) AS market_month, strftime('%Y', market_date) AS market_year 
FROM customer_purchases;
...
SELECT customer_id, strftime('%m', market_date) AS market_month, 
	strftime('%Y', market_date) AS market_year, SUM(quantity * cost_to_customer_per_qty) AS total_spent
FROM customer_purchases GROUP BY customer_id, market_month, market_year
